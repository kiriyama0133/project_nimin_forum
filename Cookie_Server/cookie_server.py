...
class Cookie(Base):
    ...
    time = Column(TIMESTAMP, default=datetime.now, comment='time')

class User(Base):
    __tablename__ = 'users'
    __table_args__ = {'comment': 'users表'}

    #邮箱和密码
    email = Column(String(50), primary_key=True, comment='email')
    password = Column(String(50), nullable=False, comment='password')
    time = Column(TIMESTAMP, default=datetime.now, comment='time')
...
class CookieService(cookie_service_pb2_grpc.CookieServiceServicer):
    ...
    def SendMail_C(self,request, context):
        try:
            ...
            request_smtp = smtp_service_pb2.SendMailRequest(email=request.email)
            if '@stu.swpu.edu.cn' in request.email:
                # Call the SendMail method
                response = stub.SendMail(request_smtp)
                logger.info(str(current_datetime) + " " + "SendMailRequest: " + str(response))
                veri = response.message
                if "邮件发送成功!" in response.status:
                    logger.info(str(current_datetime) + " " + "SendMailRequest: " + str(response))
                    return cookie_service_pb2.SendMailResponse(status="success")
                else:
                     logger.error(f"{current_datetime} - Failed to send email to{request.email}")
                     return cookie_service_pb2.SendMailResponse(status="error")
            else:
                logger.error(f"{current_datetime} - Invalid email address: {request.email}")
                return cookie_service_pb2.SendMailResponse(status="邮箱不合法")
    ...
    def Rigister(self, request, context):
        try:
            use = session.query(User).filter(User.email == request.email).first()
            if use:
                # 邮箱已存在 输出到日志
                logger.info(str(current_datetime) + " " + "Email already exists.")
                return cookie_service_pb2.RigisterResponse(status="error", message="Email already exists.")
            else:
                # 邮箱不存在，创建新用户
                new_user = User(email=request.email, password=request.password)
                session.add(new_user)
                session.commit()
                logger.info(str(current_datetime) + " " + "Email registered successfully.")
                return cookie_service_pb2.RigisterResponse(status="success", message="Email registered successfully.")
        except SQLAlchemyError as e:
            logger.error(str(current_datetime) + " " + f"Error: {e}")
    ...
    def Login(self, request, context):
        try:
            ...
            if user and user.password == request.password:
                # 密码正确，输出到日志
                logger.info(str(current_datetime) + " " + "Login successful.")
                return cookie_service_pb2.CookieResponse(status="success", message="Login successful.")
            else:  # 密码不正确，输出到日志
                logger.info(str(current_datetime) + " " + "Incorrect password.")
                return cookie_service_pb2.CookieResponse(status="error", message="Incorrect password.")
        except SQLAlchemyError as e:
            logger.error(str(current_datetime) + " " + f"Error: {e}")
def serve():
    # 创建 gRPC 服务器
